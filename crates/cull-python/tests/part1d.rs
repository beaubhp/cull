...
fn part1d_snapshot_is_stable() {
    ...
    assert_eq!(
        first,
        include_str!("snapshots/part1d_final_debug_references.json")
    );
}

#[test]
fn cpython_oracle_exposes_part1d_annotation_scope_facts() {
    let Some(python) = cpython_314() else {
        return;
    };
    let source = fixture("final").join("src/pkg/part1d.py");
...
fn type_checking_origin_overloads_and_candidates_are_recorded() {
    ...
    let test_module = output
        .modules
        .iter()
        .find(|module| module.name == "test_part1d")
    ...
    let unreferenced = output
        .definitions
        .iter()
        .find(|definition| definition.qualified_name == "pkg.part1d::unreferenced")
...
fn definition_effects_and_removal_risk_are_structural() {
    ...
    let eager = output
        .definitions
        .iter()
        .find(|definition| definition.qualified_name == "pkg.part1d::eager_annotation")
