...
use culler_python::{
    analyze_check, analyze_debug_candidates, CheckOptions, DebugCandidatesOptions,
};
...
fn check_project(root: &Path, mode: ProjectMode, allow_partial: bool) -> culler_core::CheckOutput {
...
fn debug_candidates(
    ...
    mode: ProjectMode,
    allow_partial: bool,
) -> culler_core::DebugCandidatesOutput {
...
fn schema_v3_findings_have_subjects_ids_evidence_and_reported_status() {
    ...
    assert!(dead
        .evidence
        .iter()
        .any(|evidence| evidence.kind == culler_core::EvidenceKind::NoInboundReferences));
}
