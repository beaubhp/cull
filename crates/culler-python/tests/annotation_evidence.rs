...
use culler_python::{analyze_debug_references, DebugReferencesOptions};
...
fn analyze_fixture(name: &str) -> culler_core::DebugReferencesOutput {
...
) -> culler_core::DebugReferencesOutput {
...
fn normalized_json(mut output: culler_core::DebugReferencesOutput) -> String {
    output.project_root = "<PROJECT_ROOT>".to_owned();
    format!("{}\n", serde_json::to_string_pretty(&output).unwrap())
}

fn references_named<'a>(
    output: &'a culler_core::DebugReferencesOutput,
    source_spelling: &str,
) -> Vec<&'a ReferenceFact> {
...
fn binding_kinds_for_reference(
    output: &culler_core::DebugReferencesOutput,
...
fn cpython_oracle_exposes_annotation_scope_facts() {
    ...
    let oracle = Command::new(python)
        .arg(
            workspace_root()
                .join("crates/culler-python/tests/support/cpython_definition_oracle.py"),
        )
        .arg(&source)
        .output()
        .unwrap();
...
fn type_checking_origin_overloads_and_candidates_are_recorded() {
    ...
    assert_eq!(test_module.origin_domain, culler_core::OriginDomain::Test);
    assert_eq!(
        test_module.origin_evidence,
        culler_core::OriginEvidence::CullerConfiguration
    );
    ...
    assert!(output.references.iter().any(|reference| {
        matches!(
            ...
            culler_core::Resolution::Unresolved(
                culler_core::UnresolvedReason::UnsupportedAnnotation
            )
        )
    }));
