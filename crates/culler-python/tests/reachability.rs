...
use culler_core::{
    FindingConfidence, FindingRule, FindingType, ProjectMode, RootCoverage, RootKind,
};
...
fn finding<'a>(output: &'a culler_core::CheckOutput, name: &str) -> &'a culler_core::Finding {
    output
        .findings
        .iter()
...
fn assert_no_finding(output: &culler_core::CheckOutput, name: &str) {
    assert!(
...
fn assert_no_method_findings(output: &culler_core::CheckOutput) {
    assert!(
        output.findings.iter().all(|finding| {
            !finding
                .subject
...
fn assert_no_error_diagnostics(output: &culler_core::CheckOutput) {
    assert!(
        output
            .diagnostics
            .iter()
            .all(|diagnostic| diagnostic.severity != culler_core::DiagnosticSeverity::Error),
...
[tool.culler]\nsrc = 'src'\nroot_coverage = 'complete'\nroots = ['pkg.cli:main']\n",
    ...
    assert!(output
        .diagnostics
        .iter()
        .any(|diagnostic| diagnostic.severity == culler_core::DiagnosticSeverity::Error));
}
...
def lazy_only():\n    pass\n",
    ...
    assert!(lazy_only
        .reference_phases
        .iter()
        .any(|phase| phase.phase == culler_core::ReferencePhase::LazyAnnotation));
}
...
fn unresolved_metaclass_construction_keeps_method_side_findings_review() {
    ...
    assert!(helper.uncertainty.iter().any(|uncertainty| uncertainty.kind
        == culler_core::FindingUncertaintyKind::DynamicClassConstruction));
}
