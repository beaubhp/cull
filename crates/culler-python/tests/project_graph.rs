...
use culler_core::{FindingConfidence, ProjectMode};
...
fn finding<'a>(
    output: &'a culler_core::CheckOutput,
    name: &str,
) -> Option<&'a culler_core::Finding> {
    output
        .findings
        .iter()
...
fn circular_local_imports_attach_partial_initialization_uncertainty() {
    ...
    assert!(finding
        ...
        .any(|uncertainty| uncertainty.kind
            == culler_core::FindingUncertaintyKind::PartialInitialization));
}
