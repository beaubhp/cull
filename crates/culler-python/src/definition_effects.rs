...
fn group_overloads(builder: &mut SemanticGraphBuilder, diagnostics: &mut Vec<Diagnostic>) {
    ...
    let mut pending: BTreeMap<(culler_core::ScopeId, String), Vec<DefId>> = BTreeMap::new();
...
fn warn_missing_overload_implementation(
    graph: &culler_core::SemanticGraph,
    diagnostics: &mut Vec<Diagnostic>,
    overload_name: &str,
    declarations: &[DefId],
) {
...
fn build_internal_candidates(builder: &mut SemanticGraphBuilder) {
    let inputs = {
        ...
        let mut flow_uncertainty_sets: Vec<&[culler_core::FlowUncertaintyKind]> =
            ...
            .filter(|reference| {
                reference.role == ReferenceRole::Annotation
                    && matches!(
                        ...
                        culler_core::Resolution::Unresolved(
                            UnresolvedReason::UnsupportedAnnotation
                        )
                    )
            })
...
fn mark_unsupported_reference(
    reference: &culler_core::ReferenceFact,
...
fn candidate_symbols_for_reference(
    reference: &culler_core::ReferenceFact,
) -> BTreeSet<culler_core::SymbolId> {
    let mut symbols = BTreeSet::new();
    match &reference.lexical_target {
        culler_core::Resolution::Resolved(symbol) => vec![*symbol],
        culler_core::Resolution::Ambiguous(symbols) => symbols.clone(),
        culler_core::Resolution::External | culler_core::Resolution::Unresolved(_) => Vec::new(),
    }
    .into_iter()
    .for_each(|symbol| {
...
mod tests {
    ...
    fn class_lookup_candidates_include_global_fallback_symbol() {
        ...
        let class_symbol = culler_core::SymbolId::new(1);
        let global_symbol = culler_core::SymbolId::new(2);
        let reference = culler_core::ReferenceFact {
            id: culler_core::ReferenceId::new(0),
            module,
            source_scope: culler_core::ScopeId::new(1),
            source_context: culler_core::ContextId::new(1),
    ...
    fn unresolved_local_flow_suppresses_internal_candidates() {
        ...
        let definition = builder.add_definition(culler_core::DefinitionInput {
            module,
            binding,
            scope: definition_scope,
            context: definition_context,
            kind: culler_core::DefinitionKind::Function,
            name: "target".to_owned(),
