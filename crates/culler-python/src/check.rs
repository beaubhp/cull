...
fn check_analysis(
    mode: ProjectMode,
    target_python: culler_core::PythonVersion,
...
fn apply_partial_analysis_policy(
    ...
    if !allow_partial
        || project_completeness.status != culler_core::ProjectCompletenessStatus::Partial
    {
        return;
    }
...
struct ProjectFacts<'a> {
    ...
    module_scopes: Vec<Option<culler_core::ScopeId>>,
    module_contexts: Vec<Option<culler_core::ContextId>>,
    scopes: Vec<Option<&'a culler_core::ScopeFact>>,
    contexts: Vec<Option<&'a culler_core::ContextFact>>,
    ...
    context_flow_statuses: Vec<Option<&'a culler_core::ContextFlowStatusFact>>,
...
impl<'a> ProjectFacts<'a> {
    ...
    fn module_context(&self, module: ModuleId) -> Option<culler_core::ContextId> {
    ...
    fn module_scope(&self, module: ModuleId) -> Option<culler_core::ScopeId> {
        self.module_scopes
            .get(module.as_u32() as usize)
            .copied()
            .flatten()
    ...
    fn scope(&self, scope: ScopeId) -> Option<&'a culler_core::ScopeFact> {
        self.scopes.get(scope.as_u32() as usize).copied().flatten()
    }

    fn context(&self, context: ContextId) -> Option<&'a culler_core::ContextFact> {
        self.contexts
    ...
    fn bindings_for_symbol(&self, symbol: culler_core::SymbolId) -> &[BindingId] {
    ...
    fn binding_set(&self, set: culler_core::BindingSetId) -> &[BindingId] {
        self.binding_sets
    ...
    fn flow_uncertainties(&self, set: culler_core::FlowUncertaintySetId) -> &[FlowUncertaintyKind] {
                ...
                ReferenceBindingState::Analyzed(state) => {
                    ...
                    if exact_bindings.is_empty() {
                        ...
                        if lexical_bindings.is_empty() {
                            if let culler_core::Resolution::Resolved(symbol) =
...
fn lexical_fallback_bindings(
    ...
    let culler_core::Resolution::Resolved(symbol) = reference.lexical_target else {
        return Vec::new();
    };
...
struct ReachabilityAnalysis {
    ...
    production_contexts: BTreeSet<culler_core::ContextId>,
    test_values: BTreeSet<DefId>,
    test_contexts: BTreeSet<culler_core::ContextId>,
    external_values: BTreeSet<DefId>,
    external_contexts: BTreeSet<culler_core::ContextId>,
...
impl<'a, 'b, 'c> ReachabilityBuilder<'a, 'b, 'c> {
        ...
        match self.project.root_coverage {
            Some(crate::config::RootCoverageAssertion::Complete) => {
                if production_roots == 0 || unresolved_declared || dynamic_unavailable {
                    self.diagnostics.push(Diagnostic::error(
                        "CULL_P3002",
                        "root_coverage = \"complete\" was asserted, but Culler could not validate a complete production root set",
                    ));
    ...
    fn scan_context(&mut self, domain: ReachabilityDomain, context: culler_core::ContextId) {
    ...
    fn mark_context(&mut self, domain: ReachabilityDomain, context: culler_core::ContextId) {
...
fn build_findings(
        ...
        .filter(|definition| definition.reportable)
    {
        ...
        if definition.role == culler_core::DefinitionRole::OverloadDeclaration {
            continue;
        }
        ...
        if project_completeness.status == culler_core::ProjectCompletenessStatus::Partial {
        ...
        let exports = export_refs
            .iter()
            .map(|export| culler_core::FindingExport {
...
fn build_unused_import_findings(
    ...
    for meta in metadata {
        ...
        let mut reason =
            "import binding has no may-execute reads under Culler's flow model".to_owned();
        let mut blockers = Vec::new();
        let mut uncertainty =
...
fn build_unused_local_binding_findings(
    ...
    for binding in &graph.bindings {
        ...
        let mut confidence = FindingConfidence::High;
        let mut reason =
            "local binding has no may-execute reads under Culler's flow model".to_owned();
        let mut blockers = vec![FindingBlocker {
            kind: FindingBlockerKind::RemovalRisk,
            detail:
                "assignment right-hand side may still have effects; Culler reports binding unusedness"
                    .to_owned(),
        }];
        let mut uncertainty =
...
fn build_unreachable_statement_findings(
    ...
    for range in ranges {
        ...
        let mut confidence = FindingConfidence::High;
        let mut reason = "statement range is unreachable under Culler's flow model".to_owned();
        let mut blockers = Vec::new();
...
fn owner_region(scope: &culler_core::ScopeFact, facts: &ProjectFacts<'_>) -> UncertaintyRegion {
    scope
        .owner_definition
        .and_then(|owner| facts.definition(owner))
        .map(|owner| UncertaintyRegion::definition(owner.qualified_name.clone()))
...
fn build_unused_private_method_findings(
    ...
    for meta in metas {
        ...
        let mut confidence = FindingConfidence::High;
        let mut reason =
            "private method has no resolved inbound method references under Culler's static model"
                .to_owned();
        let mut blockers = Vec::new();
        let mut uncertainty =
    ...
    let unreachable = graph
        ...
        .filter(|definition| {
            !is_definition_reachable_for_cluster(definition, reachability, mode)
                && definition.role != culler_core::DefinitionRole::OverloadDeclaration
        })
        .map(|definition| definition.id)
        .collect::<BTreeSet<_>>();
...
fn finding_reference(
    reference: &ReferenceFact,
    facts: &ProjectFacts<'_>,
) -> Option<culler_core::FindingReference> {
    let parsed = facts.module_source(reference.module)?;
    Some(culler_core::FindingReference {
        kind: match reference.role {
            culler_core::ReferenceRole::Import => FindingReferenceKind::Import,
            culler_core::ReferenceRole::ModuleAttribute => FindingReferenceKind::ModuleAttribute,
            culler_core::ReferenceRole::Export => FindingReferenceKind::Export,
...
fn origin_domain_summary(
    references: &[culler_core::FindingReference],
...
fn reference_phase_summary(
    references: &[culler_core::FindingReference],
) -> Vec<culler_core::FindingPhaseSummary> {
    let mut counts = Vec::<culler_core::FindingPhaseSummary>::new();
    for reference in references {
        if let Some(summary) = counts
            .iter_mut()
            .find(|summary| summary.phase == reference.phase)
        {
            ...
            counts.push(culler_core::FindingPhaseSummary {
                phase: reference.phase,
                count: 1,
            });
        }
...
fn reachability_for_finding(
    ...
    let mut summary =
        "no runtime path was found in Culler's static reachability graph from recognized roots"
            .to_owned();
...
fn reaching_bindings<'a>(
    ...
    let culler_core::ReferenceBindingState::Analyzed(state) = &reference.binding_state else {
        return Cow::Borrowed(&[]);
    };
    if state.reachability == LocalReachability::Unreachable {
        return Cow::Borrowed(&[]);
    ...
    if let culler_core::Resolution::Resolved(symbol) = reference.lexical_target {
...
fn evidence_for(
    finding_type: FindingType,
    inbound_references: &[culler_core::FindingReference],
    exports: &[culler_core::FindingExport],
    surface: DefinitionSurface,
...
fn confidence_for_surface(
    ...
    match surface {
        DefinitionSurface::Private => (
            ...
            "private definition has no inbound references under Culler's static model".to_owned(),
        ),
        ...
        DefinitionSurface::Statement => (
            FindingConfidence::High,
            "statement range is unreachable under Culler's flow model".to_owned(),
        ),
