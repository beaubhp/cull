...
use culler_core::{OriginEvidence, ProjectMode, PythonVersion};
...
pub enum ConfigError {
    ...
    #[error("invalid [tool.culler].mode `{0}`; expected auto, application, or library")]
    InvalidMode(String),
    #[error("invalid [tool.culler].root_coverage `{0}`; expected complete or partial")]
    InvalidRootCoverage(String),
    #[error(
        "invalid [tool.culler].target-python/target-version `{0}`; expected 3.10 through 3.15"
    )]
...
pub fn load_project_config(project_root: &Path) -> Result<ProjectConfig, ConfigError> {
    ...
    let Some(culler) = tool.culler else {
        let mut config = ProjectConfig::empty();
    ...
    let culler_test_paths = culler.tests.map(StringOrVec::into_vec);
    let (test_paths, test_path_origin_evidence) = if let Some(paths) = culler_test_paths {
        (paths, Some(OriginEvidence::CullerConfiguration))
    } else {
        (pytest_test_paths, pytest_test_path_origin_evidence)
    };

    let mode = match culler.mode.as_deref() {
        Some(value) => {
            parse_mode(value).ok_or_else(|| ConfigError::InvalidMode(value.to_owned()))?
        }
        None => ProjectMode::Auto,
    };
    let root_coverage = match culler.root_coverage.as_deref() {
        Some(value) => Some(
    ...
    let target_python = match culler.target_python.or(culler.target_version) {
        Some(value) => Some(
            value
                .parse()
    ...
    Ok(ProjectConfig {
        source_roots: culler.src.map(StringOrVec::into_vec).unwrap_or_default(),
        excludes: culler.exclude.unwrap_or_default(),
        target_python,
        mode,
        test_paths,
        ...
        dynamic_scripts,
        dynamic_gui_scripts,
        allow_partial: culler.allow_partial.unwrap_or(false),
    })
}
